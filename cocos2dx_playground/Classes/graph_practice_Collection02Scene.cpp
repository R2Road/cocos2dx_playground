...
namespace graph_practice
{
	...
	bool Collection02Scene::init()
	{
		...
		//
		{
			auto graph_view_node = GraphViewNode::create( "quadEase In", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::quadEaseIn( g_x ); } );
			graph_view_node->setPosition(
		...
		//
		{
			auto graph_view_node = GraphViewNode::create( "quadEase Out", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::quadEaseOut( g_x ); } );
			graph_view_node->setPosition(
		...
		//
		{
			auto graph_view_node = GraphViewNode::create( "quadEase In Out", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::quadEaseInOut( g_x ); } );
			graph_view_node->setPosition(
		...
		//
		{
			auto graph_view_node = GraphViewNode::create( "cubicEase In", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::cubicEaseIn( g_x ); } );
			graph_view_node->setPosition(
				visibleOrigin.x + ( visibleSize.width * ( StartRateX + ( SpacingX * 1 ) ) ) - ( graph_view_node->getContentSize().width * 0.5f )
		...
		//
		{
			auto graph_view_node = GraphViewNode::create( "cubicEase Out", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::cubicEaseOut( g_x ); } );
			graph_view_node->setPosition(
				visibleOrigin.x + ( visibleSize.width * ( StartRateX + ( SpacingX * 2 ) ) ) - ( graph_view_node->getContentSize().width * 0.5f )
				, visibleOrigin.y + ( visibleSize.height * ( StartRateY + ( SpacingY * 1 ) ) ) - ( graph_view_node->getContentSize().height * 0.5f )
			);
		...
		//
		{
			auto graph_view_node = GraphViewNode::create( "cubicEase In Out", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::cubicEaseInOut( g_x ); } );
			graph_view_node->setPosition(
				visibleOrigin.x + ( visibleSize.width * ( StartRateX + ( SpacingX * 3 ) ) ) - ( graph_view_node->getContentSize().width * 0.5f )
				, visibleOrigin.y + ( visibleSize.height * ( StartRateY + ( SpacingY * 1 ) ) ) - ( graph_view_node->getContentSize().height * 0.5f )
			);
