...
{
	LayoutChangeSequenceScene::LayoutChangeSequenceScene( const helper::FuncSceneMover& back_to_the_previous_scene_callback ) :
	...
	Scene* LayoutChangeSequenceScene::create( const helper::FuncSceneMover& back_to_the_previous_scene_callback )
	{
		auto ret = new ( std::nothrow ) LayoutChangeSequenceScene( back_to_the_previous_scene_callback );
		if( !ret || !ret->init() )
		{
			delete ret;
			ret = nullptr;
	...
	bool LayoutChangeSequenceScene::init()
	{
		if( !Scene::init() )
		{
			return false;
		...
		//
		{
			const Vec2 LayoutStartPosition( visibleSize.width * 0.5f, visibleSize.height * 0.45f );
	...
	void LayoutChangeSequenceScene::onEnter()
	{
		Scene::onEnter();

		assert( !mKeyboardListener );
		mKeyboardListener = EventListenerKeyboard::create();
		mKeyboardListener->onKeyPressed = CC_CALLBACK_2( LayoutChangeSequenceScene::onKeyPressed, this );
		getEventDispatcher()->addEventListenerWithSceneGraphPriority( mKeyboardListener, this );
	}
	void LayoutChangeSequenceScene::onExit()
	{
		assert( mKeyboardListener );
	...
	ui::Layout* LayoutChangeSequenceScene::MakeLayoutVertical(
	...
	)
	{
		...
		for( int i = 1; button_content_count >= i; ++i )
		{
			...
			button->addTouchEventListener( CC_CALLBACK_2( LayoutChangeSequenceScene::onDummyButton, this ) );
	...
	void LayoutChangeSequenceScene::onDummyButton( Ref* sender, ui::Widget::TouchEventType touchEventType )
	{
	...
	void LayoutChangeSequenceScene::onKeyPressed( EventKeyboard::KeyCode keycode, Event* /*event*/ )
	{
		...
		{
			CCASSERT( 2 <= mLayout->getChildrenCount(), "LayoutChangeSequenceScene - Swap 1st N 2nd" );
