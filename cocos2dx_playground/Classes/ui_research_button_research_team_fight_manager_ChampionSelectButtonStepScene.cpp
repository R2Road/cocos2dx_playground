...
namespace ui_research
{
	...
	{
		TeamFightManagerChampionSelectButtonStepScene::TeamFightManagerChampionSelectButtonStepScene( const helper::FuncSceneMover& back_to_the_previous_scene_callback ) :
			helper::BackToThePreviousScene( back_to_the_previous_scene_callback )
			, mKeyboardListener( nullptr )
		{}

		Scene* TeamFightManagerChampionSelectButtonStepScene::create( const helper::FuncSceneMover& back_to_the_previous_scene_callback )
		{
			auto ret = new ( std::nothrow ) TeamFightManagerChampionSelectButtonStepScene( back_to_the_previous_scene_callback );
			if( !ret || !ret->init() )
			{
				delete ret;
				ret = nullptr;
		...
		bool TeamFightManagerChampionSelectButtonStepScene::init()
		{
			if( !Scene::init() )
			{
				return false;
		...
		void TeamFightManagerChampionSelectButtonStepScene::onEnter()
		{
			Scene::onEnter();

			assert( !mKeyboardListener );
			mKeyboardListener = EventListenerKeyboard::create();
			mKeyboardListener->onKeyPressed = CC_CALLBACK_2( TeamFightManagerChampionSelectButtonStepScene::onKeyPressed, this );
			getEventDispatcher()->addEventListenerWithSceneGraphPriority( mKeyboardListener, this );
		}
		void TeamFightManagerChampionSelectButtonStepScene::onExit()
		{
			assert( mKeyboardListener );
		...
		void TeamFightManagerChampionSelectButtonStepScene::onKeyPressed( EventKeyboard::KeyCode keycode, Event* /*event*/ )
		{
