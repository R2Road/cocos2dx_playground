...
namespace step_rain_of_chaos
{
	namespace collision
	{
		...
		{
			auto target_component = child->getComponent( step_mole::CollisionComponent::GetStaticName() );
			if( !target_component )
			{
				return;
			}

			mCollisionCollection.Add( static_cast<step_mole::CollisionComponent*>( target_component ) );
		}
		void CollectionScene::removeCollision( cocos2d::Node* child )
		{
			...
			mCollisionCollection.Remove( static_cast<step_mole::CollisionComponent*>( target_component ) );
		}
		...
		Node* CollectionScene::makeBullet()
		{
			auto bullet_root_node = Node::create();
			{
				...
				// Collision Component
				bullet_root_node->addComponent( step_mole::CollisionComponent::create( radius, false, false, false ) );
			}
